...
@app.route('/get_ingestion_status/<task_id>')
@cross_origin()
...
@app.route('/get_sample_relations/<task_id>')
@cross_origin()
def get_sample_relations(task_id):
...
def search_template():
    data = request.get_json(force=True)
    return template_explorer.search_template(query=data['query'])
